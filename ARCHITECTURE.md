...
- `src/pages/` — File-based routing. `.astro`, `.md`, and `.js` files become routes. Includes `posts/`, `tags/` subdirs, `about.astro`, `archive.astro`, `[...page].astro` (pagination), `404.astro`, and `rss.xml.ts` endpoint.
- `src/components/` — Astro components: `BlogPost.astro`, `Comment.astro`, `CommentThread.astro`, `Comments.astro`, `Footer.astro`, `Header.astro`, `Navigation.astro`, `PostHistory.astro`, `Social.astro`, `ThemeIcon.astro`.
...
- `src/utils/` — Utility functions: `date.ts`, `git.ts`.
